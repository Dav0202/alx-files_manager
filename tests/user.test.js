...
describe('testing User Endpoints', () => {
  const credentials = 'Basic a3lsZUBicm92bG9reWUuY29tOkF1dGhvcml0YWgh';
  let token = '';
  let userId = '';
  const user = {
    email: 'kyle@brovlokye.com',
    password: 'Authoritah!',
  };
  ...
  describe('POST /users', () => {
    ...
    it('fails to create user because password is missing', async () => {
      const user = { email: 'kyle@brovlokye.com', };
      const response = await request(app).post('/users').send(user);
      const body = JSON.parse(response.text);
    ...
    it('fails to create user because email is missing', async () => {
      const user = { password: 'Authoritah!', };
      const response = await request(app).post('/users').send(user);
      const body = JSON.parse(response.text);
