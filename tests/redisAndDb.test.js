...
describe('Testing the clients for MongoDB and Redis', () => {
  ...
  describe('db Client', () => {
    ...
    it('shows number of user documents', async () => {
      ...
      await dbClient.usersCollection.insertOne({ name: 'Peter' });
    ...
    it('shows number of file documents', async () => {
      ...
      await dbClient.filesCollection.insertOne({ name: 'FirstFile' });
      await dbClient.filesCollection.insertOne({ name: 'Second File' });
      expect(await dbClient.nbUsers()).to.equal(2);
    });
  });
});
